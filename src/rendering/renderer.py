...
import numpy as np
import sys
sys.path.append('../../../../Users/baruh/Downloads/New folder')
...
class Renderer:
    ...
    def draw_checkpoints(self, checkpoints):
        ...
        Start/finish line is checkered. Sector lines are colored like F1 timing.
        """
        for cp in checkpoints:
            inner_scr = self.world_to_screen(cp['inner'])
            outer_scr = self.world_to_screen(cp['outer'])
            ...
            if cp['is_start_finish']:
                inner = np.array(inner_scr, dtype=float)
                outer = np.array(outer_scr, dtype=float)
                line = outer - inner
                ...
                stripe_count = max(8, int(length // 8))
                checker_width = 10
                for i in range(stripe_count):
                    a = inner + line * (i / stripe_count)
                    b = inner + line * ((i + 1) / stripe_count)
                    for side in (-1, 1):
                        color = (
                            (255, 255, 255)
                            if (i + (1 if side > 0 else 0)) % 2 == 0
                            else (20, 20, 20)
                        )
                        pygame.draw.polygon(
                            self.screen,
                            ...
                            [
                                tuple(a),
                                tuple(b),
                                tuple(b + normal * checker_width * side),
                                tuple(a + normal * checker_width * side),
                            ],
                        )
                pygame.draw.line(self.screen, (255, 255, 255), inner_scr, outer_scr, 2)
            else:
                color_idx = cp['index'] % len(SECTOR_COLORS)
                pygame.draw.line(
                    self.screen,
                    SECTOR_COLORS[color_idx],
                    inner_scr,
                    outer_scr,
                    2,
                )
    ...
    def draw_car(self, car, touching_wall=False, touching_car=False):
        ...
        # Draw car body
        body_color = getattr(car, "render_color", RENDER.car_color)
        if getattr(car, "is_static_control", False) and not hasattr(car, "render_color"):
            body_color = (70, 170, 255)
        elif getattr(car, "is_main_player", False) and not hasattr(car, "render_color"):
            body_color = (230, 50, 50)
        elif not hasattr(car, "render_color"):
    ...
    def draw_minimap(self, track, cars):
        """Draw a compact live race map and leaderboard."""
        cars = list(cars)
        if not cars:
            return
        ...
        panel_w = 250
        panel_h = 250
        panel_x = RENDER.screen_width - panel_w - 10
        panel_y = RENDER.screen_height - panel_h - 10
        map_w = 220
        map_h = 130
        padding = 10
        line_h = 14
        ...
        title_surface = minimap_font.render("MINI MAP", True, (255, 255, 255))
        self.screen.blit(
            title_surface,
            (panel_x + (panel_w - title_surface.get_width()) // 2, panel_y + 8),
        )
        ...
        scale = min(
            (map_w - 2 * padding) / span[0],
            (map_h - 2 * padding) / span[1],
        )
        drawn_w = span[0] * scale
        map_x = panel_x + (panel_w - drawn_w) / 2
        map_y = panel_y + 34

        def to_map(pos):
            pos = np.asarray(pos, dtype=float)
            x = map_x + (pos[0] - min_xy[0]) * scale
            y = map_y + map_h - padding - (pos[1] - min_xy[1]) * scale
            return int(x), int(y)
        ...
        if len(points) > 1:
            pygame.draw.lines(
                ...
                (130, 130, 130),
                True,
                [to_map(p) for p in points],
                2,
            )

        checkpoints = track.get_checkpoint_positions(1)
        if checkpoints:
            cp = checkpoints[0]
            pygame.draw.line(
                self.screen,
        ...
        for car in cars:
            color = getattr(car, "render_color", RENDER.car_color)
            pos = to_map(car.position)
            pygame.draw.circle(self.screen, color, pos, 5)
            pygame.draw.circle(self.screen, (255, 255, 255), pos, 5, 1)

        ordered = sorted(
            cars,
            key=lambda c: getattr(c, "minimap_race_progress", 0.0),
            reverse=True,
        )
        legend_y = panel_y + 174
        for index, car in enumerate(ordered, start=1):
            color = getattr(car, "render_color", RENDER.car_color)
            label = getattr(car, "minimap_label", f"car {car.car_id}")
            laps = int(getattr(car, "minimap_laps", 0))
            speed = float(getattr(car, "speed", 0.0)) * 3.6
            y = legend_y + (index - 1) * line_h
            text = f"P{index} {label} | {speed:3.0f}km/h | L{laps}"
            if minimap_font.size(text)[0] > panel_w - 44:
                max_chars = max(8, 24 - len(f"P{index}  | {speed:3.0f}km/h | L{laps}"))
                label = label[:max_chars].rstrip() + "..."
                text = f"P{index} {label} | {speed:3.0f}km/h | L{laps}"
            pygame.draw.circle(self.screen, color, (panel_x + 18, y + 7), 4)
            surface = minimap_font.render(
                text,
                True,
                (230, 230, 230),
            )
            self.screen.blit(surface, (panel_x + 30, y))
